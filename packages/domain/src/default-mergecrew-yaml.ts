/**
 * Canonical lifecycle YAML for a fresh mergecrew project — the full
 * 8-stage roster (Discovery → PM → Implementation → QA → DeployDev →
 * Observation) with the 10 specialized agent kinds the system was
 * originally designed around.
 *
 * Lives in `@mergecrew/domain` (not `@mergecrew/config-yaml`) so the
 * stock-template catalog (`lifecycle-templates.ts`) can pull it in
 * without a circular dependency on config-yaml's parser. The
 * `defaultConfig()` wrapper in `@mergecrew/config-yaml` re-exports
 * this constant and adds the cached parse, preserving the existing
 * public surface.
 *
 * Until #529 ("docs alignment + remove orphan default.ts once roster
 * ships") picks a single source of truth, this file IS the source.
 * The `roster` stock template (#515) reads from here directly; any
 * edit here changes both the template's default and the
 * `defaultConfig()` result downstream.
 */
