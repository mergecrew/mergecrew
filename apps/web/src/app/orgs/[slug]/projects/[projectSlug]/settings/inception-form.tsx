  ...
  if (!hasRepo) {
    return (
      <p className="text-sm text-muted">
        Connect a repository above first. Inception clones the repo (read-only) and detects the
        stack so the setup wizard can pre-fill defaults.
      </p>
    );
  }
    ...
    <div className="space-y-3">
      <div>
        ...
        <p className="mt-2 text-xs text-muted">
          Clones the repo into a temp workspace, scans for frameworks, scripts, and deploy
          workflows. No writes.
        </p>
      </div>
      ...
      {result && (
        <div className="space-y-3">
          ...
          <details className="rounded border p-2 text-sm ">
            <summary className="cursor-pointer font-medium">Draft mergecrew.yaml</summary>
            <textarea
              readOnly
              spellCheck={false}
              className="mt-2 w-full rounded border bg-bg p-2 font-mono text-xs "
              rows={20}
              value={result.draftYaml}
            />
            <p className="mt-1 text-xs text-muted">
              Copy this into <code>mergecrew.yaml</code> at the repo root. The first daily run will
          ...
          <ul className="space-y-1">
            {frameworks.map((f) => (
              <li key={f.kind}>
                ...
                <span className="block text-xs text-muted">{f.evidence}</span>
              </li>
            ))}
          </ul>
        )}
        ...
        {scripts.length === 0 ? (
          ...
          <ul className="space-y-1">
            {scripts
              ...
              .map((s) => (
                <li key={s.name}>
                  <span className="font-mono text-xs text-muted">{s.kind}</span>{' '}
                  <span className="font-medium">{s.name}</span>
                  <span className="block truncate font-mono text-xs text-muted">{s.cmd}</span>
                </li>
              ))}
          </ul>
        )}
          ...
          <ul className="space-y-1">
            {deployCandidates.map((w) => (
              <li key={w.path}>
                ...
                <span className="block text-xs text-muted">triggers: {w.events.join(', ')}</span>
