...
export class OrgService {
  ...
  async onboarding(orgSlug: string): Promise<{
    ...
    const data = await this.prisma.withTenant(t.organizationId, async (tx) => {
      ...
      // Lifecycle row count for the first non-demo project. Every YAML
      // save creates a new (projectId, version) row, so >1 means the
      // operator has applied something past the autogenerated default —
      // covers both "applied a stock template" (#394) and "hand-edited
      // the YAML". Cheap signal that the operator made a choice.
      const lifecycleVersions = projects[0]
        ? await tx.lifecycle.count({ where: { projectId: projects[0].id } })
        : 0;
      return { llm, project: projects[0] ?? null, lifecycleVersions };
    });
    ...
    const steps = [
      ...
      {
        key: 'lifecycle_template' as const,
        label: 'Pick a lifecycle template',
        // Threshold is `>1` rather than `>0`: project creation seeds a
        // row at v1, so the operator has done nothing until a v2 exists.
        status:
          data.project && data.lifecycleVersions > 1
            ? ('complete' as const)
            : ('pending' as const),
        actionUrl: projectPath ? `${projectPath}/lifecycle` : `/orgs/${orgSlug}/projects/new`,
      },
    ];
